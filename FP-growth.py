...
import time
fbdic = dict([line.strip().split('\t') for line in open("data/dictionary.txt").readlines()])
class treeNode:
...
if __name__ == "__main__":
    ...
    # # 足球射门事件数据集
    start = time.time()
    parsedDat = [line.split() for line in open('data/footballevents0.txt').readlines()]
    initSet = createInitSet(parsedDat)
    myFPtree, myHeaderTab = createTree(initSet,100000)

    myFreList = []
    mineTree(myFPtree, myHeaderTab, 100000, set([]), myFreList)
    print '\n\n###FP-growth根据频繁项集发现进球特征'
    for item in myFreList:
        print item
        # if item.intersection('0') and len(item)==2:
        #     for i in item:
        #         if i != '1' and int(i) > 120:
        #             print "进球: " + str(fbdic[i])
        #         elif i != '1' and int(i) < 120:
        #             print "时间: " + str(i)
